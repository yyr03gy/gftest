...
/usr/bin/expect<<-EOF
set timeout -1
spawn scp -r -o StrictHostKeyChecking=no -P ${port} ${HOME}/gftest/MySql镜像制作 ${user}@${host}:${remote_dir}
expect "*password:"
send "${passwd}\r\n"
expect eof
spawn ssh -o StrictHostKeyChecking=no -p ${port} ${user}@${host} /bin/bash ${remote_dir}/MySql镜像制作/autobuild.sh
expect "*password:"
send "${passwd}\r\n"
expect eof
exit
